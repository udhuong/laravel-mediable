...
The ``MediaUpload`` will perform a number of validation checks on the source file. If any of the checks fail, a ``UdHuong\Mediable\MediaUploadException`` will be thrown with a message indicating why the file was rejected.
...
If the file does not pass validation, an instance of ``UdHuong\Mediable\MediaUploadException`` will be thrown
...
The callback takes two params, ``$model`` an instance of ``UdHuong\Mediable\Media`` the current model and ``$source`` an instance of ``UdHuong\Mediable\SourceAdapters\SourceAdapterInterface`` the current source.
